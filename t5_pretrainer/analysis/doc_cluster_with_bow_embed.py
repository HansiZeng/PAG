...
qrel_paths = ["./data/msmarco-full/TREC_DL_2019/qrel.json",
...
def get_args():
    ...
    parser.add_argument("--lex_docid_to_smtid_path",
                        default="./data/experiments-splade/t5-splade-0-12l/top_bow/docid_to_tokenids.json",
                        type=str)
    parser.add_argument("--model_dir",
                        default="./data/experiments-full-lexical-ripor/lexical_ripor_direct_lng_knp_seq2seq_1/",
                        type=str)
    parser.add_argument("--out_dir",
                        default="./t5_pretrainer/analysis/doc_cluster_images/",
                        type=str)
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(0, len(labels), 32), total=len(labels)//32):
        batch_lexical_encodings = torch.LongTensor(doc_lex_encodings[i:i+32]).cuda()
        batch_reps = model.semantic_decode(batch_lexical_encodings).mean(dim=1).detach().cpu().numpy()
    ...
    plt.title(f"t-SNE visualization for bow embedding")
    plt.savefig(os.path.join(args.out_dir, f"doc_cluster_by_bow_embed_v2.jpg"), format='jpg', dpi=300)
    plt.show()
