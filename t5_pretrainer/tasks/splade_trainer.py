...
from .evaluator import SparseApproxEvalWrapper
from ..utils.metrics import init_eval

@dataclass
class Splade_TrainingArgs(TrainingArguments):
    ...
    def __post_init__(self):
        ...
        assert "query_reg" in self.task_names and "doc_reg" in self.task_names
        assert set(self.task_names) == set(self.ln_to_weight.keys()), (set(self.task_names), set(self.ln_to_weight.keys()))
        
@dataclass
class TermEncoder_TrainingArgs(TrainingArguments):
    task_names: Optional[list] = field(default=None)
    ln_to_weight: Dict[str, float] = field(default=None)
    num_tasks: Optional[int] = field(default=None, init=False)
    full_rank_eval_qrel_path: Optional[str] = field(default=None)
...
class SpladeTrainer(Trainer):
    def __init__(self, 
                ...
                eval_config=None,
                eval_metrics: List[str] = field(default=["MRR@10","recall@10", "recall@100", "recall@200"]),
                **kwargs):
    ...
    def evaluate(
        ...
        for metric in self.eval_metrics:
            out_metrics[f"eval/{metric}"] = init_eval(metric)(dict(run), self.full_rank_qrel)
        ...
        self.control = self.callback_handler.on_evaluate(self.args, self.state, self.control, out_metrics)
